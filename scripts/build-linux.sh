#!/bin/bash
# Generic Linux build script for simple-tftpd
...
[Unit]
Description=Simple TFTP Daemon - A lightweight and secure TFTP server
...
User=simple-tftpd
Group=simple-tftpd
ExecStart=/usr/local/bin/simple-tftpd --daemon
Restart=always
RestartSec=5
...
EOF
        ...
        sudo useradd --system --no-create-home --shell /bin/false simple-tftpd 2>/dev/null || true
        ...
        print_status "Use 'sudo systemctl start simple-tftpd' to start the service"
    fi
}
...
show_usage() {
    cat << EOF
simple-tftpd - Linux Build Script
